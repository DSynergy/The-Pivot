...
Visit us on Heroku! https://travel-home-turing.herokuapp.com/
...
It’s a pain to enter my options for EVERY purchase. Allow a user to store their options in their account, so when they purchase an listing the right values are pre-selected (but can still be changed).
Gallery
...
So instead of http://www.example.com/billys-bbq/listings pointing to the listings belonging to the business Billy’s BBQ, allow http://billys-bbq.example.com/listings to be used instead.
